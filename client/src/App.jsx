...
function Sessions({ subject, onChanged }) {
  ...
  useEffect(() => { api(`/api/classes/${subject.id}/sessions`).then(setSessions).catch(e => setError(e.message)); }, [subject]);
  const act = async (session, kind) => {
    ...
    try { await api(`/api/classes/${subject.id}/${segment}${withDateInPath ? `/${session.date}` : ''}`, { method, ...(!withDateInPath && { body: JSON.stringify({ date: session.date }) }) }); onChanged(); } catch (e) { setError(e.message); }
  };
...
function WeekTimeline({ refreshToken, onChanged }) {
  ...
  const act = async session => {
    ...
    try {
      await api(`/api/classes/${session.classId}/absences${session.missed ? `/${session.date}` : ''}`, { method: session.missed ? 'DELETE' : 'POST', ...(!session.missed && { body: JSON.stringify({ date: session.date }) }) });
      await onChanged(); await loadWeek();
