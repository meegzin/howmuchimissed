...
const emptyClass = { name: '', workloadHours: '', meetingHours: '', priorAbsences: 0, weekdays: [], scheduleTimes: {} };
const toPayload = form => ({ name: form.name, totalMinutes: Math.round(Number(form.workloadHours) * 60), meetingMinutes: Math.round(Number(form.meetingHours) * 60), priorAbsences: Number(form.priorAbsences), schedules: form.weekdays.map(weekday => ({ weekday, startTime: form.scheduleTimes[weekday] || '08:00' })) });
const fromClass = item => ({ name: item.name, workloadHours: item.totalMinutes / 60, meetingHours: item.meetingMinutes / 60, priorAbsences: item.priorAbsences || 0, weekdays: item.schedules.map(value => value.weekday), scheduleTimes: Object.fromEntries(item.schedules.map(value => [value.weekday, value.startTime])) });
...
function ClassDetails({ subject, semester, onChanged }) {
  const percent = absencePercent(subject);
  return <div className={`class-details status-${subject.status}`}><div className="detail-heading"><div><span className="status">{statusText[subject.status]}</span><p>{subject.schedules.map(item => `${weekdays.find(day => day.value === item.weekday)?.label} as ${item.startTime}`).join(' / ')}</p></div><strong>{subject.totalMeetings} aulas no total</strong></div><div className="metrics"><div><strong>{subject.remainingMeetings}</strong><span>encontros que ainda pode faltar</span></div><div><strong>{formatMinutes(subject.remainingMinutes)}</strong><span>margem disponível</span></div><div><strong>{subject.absenceCount}</strong><span>faltas registradas</span></div></div><div className="progress-copy"><span>Uso do limite de 25%</span><strong>{Math.round(percent)}%</strong></div><div className="progress"><i style={{ width: `${percent}%` }}/></div><Sessions semester={semester} subject={subject} onChanged={onChanged}/></div>;
}
...
export default function App() {
  ...
  const load = async () => { const data = await api('/api/planner'); setSemester(data.semester); setClasses(data.classes); };
